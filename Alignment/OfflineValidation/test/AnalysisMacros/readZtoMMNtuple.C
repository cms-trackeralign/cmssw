...
void readTree(const char* filename) {
  ...
  TH1F* hDeltaPtOverPt =
      new TH1F("deltaPtOverPt", "#Delta p_{T}/p_{T}; (p^{reco}_{T} - p^{gen}_{T})/p^{gen}_{T};n. muons", 100, -0.1, 0.1);
  ...
  TH1F* hDeltaEta = new TH1F("deltaEta", "#Delta #eta; #eta^{reco}-#eta^{gen}; n. muons", 100, -0.001, 0.001);
  TH1F* hDeltaPhi = new TH1F("deltaPhi", "#Delta #phi; #phi^{reco}-#phi^{gen}; n. muons", 100, -0.001, 0.001);

  TH1F* h3 =
      new TH1F("deltaMass", "#Delta M(#mu#mu); M^{reco}_{#mu#mu} -M^{gen}_{#mu#mu} [GeV]; n.events", 100, -10., 10.);

  TH1F* hDeltaSagitta =
      new TH1F("deltaSagitta", "#delta_{sagitta};per muon #delta_{s} [GeV^{-1}];n. muons", 100, -50e-4, 50e-4);
