...
/************************************************/
{
  ...
  histos[0]->GetYaxis()->SetRangeUser(1., extrema.second * 1.3);
  ...
  /*
  if( (TString(histos[0]->GetName())).Contains("Diff") ||
      (TString(histos[0]->GetName())).Contains("Ratio") ){    
    rp->GetUpperPad()->cd()->SetLogy();
  }
  */
